...
public class SynchroExB
  ...
  public List<Rect> rects = Collections.synchronizedList(new ArrayList<Rect>());
  ...
  public void startProgramThread() {
	...
	while (true) {
	  ...
	  //Attempt 1 : but it blocks for too long and we can't add!
	/*
	  synchronized (rects) {
	  ...
	  */
	  ...
	  //Attempt 2 : remove one at a time
	  synchronized (rects) {
	  if (rects.size() > 0) {
	  rects.remove(0);
	  }
